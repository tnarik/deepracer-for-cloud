...
if [[ -n "${OPT_WIPE}" ]];
then
    MODEL_DIR_S3=$(eval aws ${DR_LOCAL_PROFILE_ENDPOINT_URL} s3 ls s3://${DR_LOCAL_S3_BUCKET}/${NEW_UPLOAD_MODEL} )
    if [[ -n "${MODEL_DIR_S3}" ]];
    then
    ...
    eval aws ${DR_LOCAL_PROFILE_ENDPOINT_URL} s3 rm s3://${DR_LOCAL_S3_BUCKET}/${NEW_UPLOAD_MODEL} --recursive
fi
